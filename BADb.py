...
def analizer():
    ...
    while True:
        ...
        if parsed_json['data'] == "1":
            ...
            sc.send(respuesta)
            log.write("Enviado clima de sensor " + elegido + " a Analizador\n")
        ...
        if parsed_json['data'] == "2":
            ...
            sc.send(respuesta)
            log.write("Enviado tractor " + elegido + " a Analizador\n")
        if parsed_json['data'] == "3":
            ...
            sc.send(respuesta)
            log.write("Enviado satelite " + elegido + " a Analizador\n")
        if parsed_json['data'] == "4":
            result = db.query("select * from "+ elegido.lower() +"  where id=(select MAX(id) from "+elegido.lower()+")")
            for row in result:
                respuesta = json.dumps({"precio": row ['precio']})
            sc.send(respuesta)
            log.write("Enviado cotizacion de "+elegido+"\n")
        sc.close()
    log.close()
    s.close()
